...
import {staticMode,assetPath} from '@/lib/runtime';
import {saveLocalBuild,readLocalBuilds,deleteLocalBuild,encodeBuild,decodeBuild} from '@/lib/local-builds';
...
async function catalog(c: Category) { if (!cache[c]) {
    const r = await fetch(assetPath('/data/' + c + '.json'));
    if (!r.ok)
...
export default function Configurator() {
    ...
    const refresh = async () => { if(staticMode){setSaved(readLocalBuilds());return;} const r = await fetch('/api/riguno'); if (r.ok) {
    ...
    useEffect(() => { void run(async () => { const r = await fetch(assetPath('/data/index.json')); setMeta(await r.json()); let ids = {}; try {
    ...
    } const shared=new URLSearchParams(location.hash.slice(1)).get('config');if(shared){const decoded=decodeBuild(shared);ids=decoded.parts;setName(decoded.name);}
 const id = new URL(location.href).searchParams.get('build'); if (id && !staticMode) {
        ...
        setName(d.name);
    } setBuild(await resolve(ids)); setReady(true); }); void run(refresh); }, []);
    useEffect(() => { if (ready)
    ...
    useEffect(() => { if(ready)localStorage.setItem('riguno-prefs', JSON.stringify(prefs)); const q = matchMedia('(prefers-color-scheme: dark)'); const apply = () => { document.documentElement.classList.toggle('dark', prefs.theme === 'dark' || prefs.theme === 'system' && q.matches); }; apply(); q.addEventListener('change', apply); return () => q.removeEventListener('change', apply); }, [prefs, ready]);
    useEffect(() => { if (!picker)
    ...
    useEffect(() => { if(staticMode){setPrices({offers:[],history:[],benchmarks:[]});setPriceError('Prix automatiques non connectés sur cette version GitHub Pages. Les liens marchands restent disponibles.');return;} if (!detail)
    ...
    useEffect(() => { if(staticMode)return;let active = true; const items = Object.values(build); if (!items.length) {
    ...
    const save = async (shared = false) => { if (!chosen) {
        ...
        return;
    } if(staticMode){const portable={schemaVersion:1,name,parts:ids()};if(shared){const link=new URL(assetPath('/'),location.origin);link.hash='config='+encodeBuild(portable);try{await navigator.clipboard.writeText(link.href);toast.success('Lien copié : il contient les composants et le nom de la configuration.');}catch{toast.message(link.href,{duration:30000});}}else{saveLocalBuild(portable);await refresh();toast.success('Configuration enregistrée sur cet appareil.');}return;} if (!user) {
        setAccount(true);
        return;
    ...
    return <><Toaster richColors/><header className="topbar"><a href={assetPath("/")} className="brand"><img src={assetPath("/favicon.svg")} width="36" height="36" alt=""/>riguno<span className="brand-dot">.</span></a><nav><button className={view === 'build' ? 'active' : ''} onClick={() => setView('build')}>Configurateur</button><button onClick={() => setAccount(true)}>Mes configurations</button><a href="https://github.com/ChammIzO/riguno" target="_blank" rel="noreferrer">Open source <ArrowUpRight size={14}/></a></nav><div className="top-actions"><button className="icon-button" aria-label="Pays et préférences" onClick={() => setSettings(true)}><Globe size={19}/><span>{prefs.region.toUpperCase()} · {prefs.currency}</span></button><button className="icon-button" aria-label="Changer de thème" onClick={() => setPrefs({ ...prefs, theme: prefs.theme === 'dark' ? 'light' : 'dark' })}>{prefs.theme === 'dark' ? <Sun size={19}/> : <Moon size={19}/>}</button><button className="account-button" onClick={() => setAccount(true)}><User size={17}/>{staticMode?'Mes configurations':user ? 'Mon compte' : 'Connexion'}</button></div></header>
 <main>{staticMode&&<div className="test-banner"><span>Version de test · GitHub Pages</span><span>Sauvegardes sur cet appareil · <a href="https://github.com/ChammIzO/riguno/issues" target="_blank" rel="noreferrer">Signaler un problème ↗</a></span></div>}<div className="heading"><div><div className="eyebrow">L’ATELIER PC <span>/</span> CONFIGURATEUR</div><h1>Chaque pièce. <em>Votre PC.</em></h1><p>Composez votre configuration et vérifiez ce qui fonctionne ensemble.</p></div><button className="secondary" onClick={() => run(async () => { setBuild(await (await fetch(assetPath('/data/defaults.json'))).json()); setName('Atelier AM5'); toast.success('Exemple chargé · consulter les points à vérifier.'); })}><Layers size={17}/>Charger un exemple</button></div>
